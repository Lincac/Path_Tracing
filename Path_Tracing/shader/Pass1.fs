...
/*
void World(Ray ray,inout HitReCoord hitinfo){

    // 球体
    {
        Material material;
        material.baseColor = vec3(1, 0.73, 0.25);
        ...
        if(t < hitinfo.t && t > 0){
            ...
            hitinfo.t = t;
            hitinfo.position = position;
            hitinfo.normal = normalize((position - sphere.position) / sphere.radius);
    ...
    // 平面
    {
        ...
        Triangle plane1;
        plane1.p1 = vec3(-3,-1,3);
        plane1.p2 = vec3(3,-1,3);
        plane1.p3 = vec3(3,-1,-3);
        plane1.material = material;

        float t1 = HitTriangle(ray,plane1);
        if(t1 < hitinfo.t && t1 > 0){
            vec3 position = ray.origin + t1 * ray.direction;

            hitinfo.hit = true;
            hitinfo.t = t1;
            hitinfo.position = position;
            vec3 N = normalize(cross(plane1.p2 - plane1.p1,plane1.p3 - plane1.p1));
            if(dot(N,ray.direction) > 0.) N = -N;
            hitinfo.normal = N;
            hitinfo.material = plane1.material;
        }

        Triangle plane2;
        plane2.p1 = vec3(-3,-1,3);
        plane2.p2 = vec3(-3,-1,-3);
        plane2.p3 = vec3(3,-1,-3);
        plane2.material = material;

        float t2 = HitTriangle(ray,plane2);
        if(t2 < hitinfo.t && t2 > 0){
            vec3 position = ray.origin + t2 * ray.direction;

            hitinfo.hit = true;
            hitinfo.t = t2;
            hitinfo.position = position;
            vec3 N = normalize(cross(plane2.p2 - plane2.p1,plane2.p3 - plane2.p1));
            if(dot(N,ray.direction) > 0.) N = -N;
            hitinfo.normal = N;
            hitinfo.material = plane2.material;
        }
    }
}
*/
...
vec3 PathTracing(Ray ray,inout HitReCoord hitinfo){
    ...
    for(int i=0;i<MAXBounds;i++){
        ...
        //vec3 L = toNormalHemisphere(SampleHemisphere(xi_1,xi_2),hitinfo.normal); // 均匀采样

        //vec3 L = SampleCosineHemisphere(xi_1,xi_2,hitinfo.normal); // 漫反射重要性采样
        
        // float alpha = max(0.001, hitinfo.material.roughness * hitinfo.material.roughness);
        // vec3 L = SampleGTR2(xi_1,xi_2,V,N,alpha); // 镜面反射重要性采样

        // float alpha = mix(0.1, 0.001, hitinfo.material.clearcoatGloss);
        // vec3 L = SampleGTR1(xi_1,xi_2,V,N,alpha); // 清漆反射重要性采样  返回pdf为清漆颜色
