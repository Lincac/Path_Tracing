...
Triangle getTriangle(int i){
    ...
    temp.p1 = texelFetch(Triangles,offset + 0).xyz;
    temp.p2 = texelFetch(Triangles,offset + 1).xyz;
    temp.p3 = texelFetch(Triangles,offset + 2).xyz;

    temp.n1 = texelFetch(Triangles,offset + 3).xyz;
    temp.n2 = texelFetch(Triangles,offset + 4).xyz;
    temp.n3 = texelFetch(Triangles,offset + 5).xyz;

    temp.material.emissive = texelFetch(Triangles,offset + 6).xyz;
    temp.material.baseColor = texelFetch(Triangles,offset + 7).xyz;
    ...
    temp.material.subsurface = param1.x;
    temp.material.metallic = param1.y;
    temp.material.specular = param1.z;
    temp.material.specularTint = param2.x;
    temp.material.roughness = param2.y;
    temp.material.anisotropic = param2.z;
    temp.material.sheen = param3.x;
    temp.material.sheenTint = param3.y;
    temp.material.clearcoat = param3.z;
    temp.material.clearcoatGloss = param4.x;

    // temp.material.IOR = param4.y;
    // temp.material.transmission = param4.z;
...
BVHNode getBVHNode(int i){
    ...
    temp.AA = vec3(texelFetch(BVHNodes,offset + 2).xyz);
    temp.BB = vec3(texelFetch(BVHNodes,offset + 3).xyz);
...
float HitAABB(Ray ray,vec3 aa,vec3 bb){
    ...
    vec3 vecmin = min(t0,t1);
    vec3 vecmax = max(t0,t1);

    float tmin = max(vecmin.x,max(vecmin.y,vecmin.z));
    float tmax = min(vecmax.x,min(vecmax.y,vecmax.z));

    return (tmax >= tmin) ? (tmin > 0.) ? tmin : tmax : -1;
}

void HitTriangles(Ray ray,int starti,int endi,inout HitReCoord hitinfo){
    for(int i=starti;i<endi;i++){
        Triangle temp = getTriangle(i);
        HitTriangle(ray,temp,hitinfo);
    }
}
...
void HitBVH(Ray ray,inout HitReCoord hitinfo){
    ...
    while(sp > 0){
        ...
        if(node.n > 0.){
            ...
            HitTriangles(ray,starti,endi,hitinfo);
        }
...
float GTR1(float NdotH, float a) {  // 法线分布，决定有多少法线和当前法线位于同一方向
    if (a >= 1) return 1/PI;
    float a2 = a*a;
...
float HDR_pdf(vec3 L,int Resolution) {
    vec2 uv = toSphericalCoord(normalize(L));   // 方向向量转 uv 纹理坐标

    float pdf = texture2D(hdrCache, uv).b;      // 采样概率密度
    float theta = PI * (0.5 - uv.y);            // theta 范围 [-pi/2 ~ pi/2] ?错误？ 没错，只采样上半球hdr
    //float theta = PI * (1 - uv.y);            // theta 范围 [-pi/2 ~ pi/2]
    float sin_theta = max(sin(theta), 1e-10);

    // 球坐标和图片积分域的转换系数
    float p_convert = float(Resolution * Resolution / 2) / (2.0 * PI * PI * sin_theta);  
    ...
    for(int i=0;i<MAXBounds;i++){
        ...
        if(dot(N,hdrRay.direction) > 0.0){
            ...
            if(!hdrHit.hit){
                ...
                // 采样光源
                vec3 hdr = hdrColor(L);
                float hdr_pdf = HDR_pdf(L,hdrResolution);

                // 采样brdf
                vec3 brdf = BRDF(V,N,L,hitinfo.material);
                float brdf_pdf = BRDF_Pdf(V,N,L,hitinfo.material);
        ...
        vec3 L = SampleBRDF(xi_1,xi_2,xi_3,V,N,hitinfo.material); // 整合采样
...
void main(){
    ...
    Ray ray;
    ray.origin = camPos;
    ray.direction = normalize(vec3(uv + offset,-1.5)) * rotateX(-0.3);
