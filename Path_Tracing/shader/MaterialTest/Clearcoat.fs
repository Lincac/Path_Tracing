...
vec3 PathTracing(Ray ray,inout HitReCoord hitinfo){
    ...
    for(int i=0;i<MAXBounds;i++){
        ...
        float alpha_GTR1 = mix(0.1, 0.001, hitinfo.material.clearcoatGloss);
        vec3 L = SampleGTR1(xi_1, xi_2, V, N, alpha_GTR1); // 出射光线
        ...
        float Dr = GTR1(dot(N,H), mix(0.1, 0.001, hitinfo.material.clearcoatGloss));
        float brdf_pdf = Dr * dot(N,H) / (4.0 * dot(L, H));
